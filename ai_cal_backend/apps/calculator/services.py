...
def analyze_image(img: Image, listVars: dict):
    model = genai.GenerativeModel(model_name="gemini-1.5-flash")
    stringVars = json.dumps(listVars, ensure_ascii=False)
    prompt = (
        ...
        f"Here is a dictionary of user-assigned variables. If the given expression has any of these variables, use its actual value from this dictionary accordingly: {stringVars}. "
        f"DO NOT USE BACKTICKS OR MARKDOWN FORMATTING. "
        f"PROPERLY QUOTE THE KEYS AND VALUES IN THE DICTIONARY FOR EASIER PARSING WITH Python's ast.literal_eval."
    )
    aiResponse = model.generate_content([prompt, img])
    print(aiResponse.text)
    answers = []
    try:
        answers = ast.literal_eval(aiResponse.text)
    except Exception as e:
        print(f"issue in parsing airesponse from Api: {e}")
